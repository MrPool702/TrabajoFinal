...
def leerDatos():
  with open("JorgeLuisEntregas.txt") as file:
    lines = file.readlines()
...
misAlmacenes = almacenes[56:69]

def AlgoritmoJorgeluis(grafo, almacenes, puntosEntrega):
  cont = 0
  with open("puntosJorgeluisVersion2.txt", mode= "w") as file:
    for i in almacenes:
      _, distancia = dijkstra(grafo, i.getID(), puntosEntrega[cont])
      file.write("Almacen " + str(i.getID()) + " :\n")
      for j in puntosEntrega[cont]:
        file.write("Punto de Entrega " + str(j) + " Distancia: " + str(distancia[j]) + "\n")
      cont += 1
  

AlgoritmoJorgeluis(grafo,misAlmacenes,distribucion)
